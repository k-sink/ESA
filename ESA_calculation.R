...
# using raw data, time series for daily precipitation, potential evapotranspiration, and actual 
# evapotranspiration, calculate the ESA statistic
# includes filter design, decimation, and derivative analysis 
# results in 88 ESA, 88 dESA for each of the 747 individual watersheds
...
###############################################
### MACH TIME SERIES DATA ###
###############################################
# simplify MACH data files and add SITENO column

# MACH data, csv files for each individual watershed 
# files contain DATE, AET, DAYL, SRAD, OBSQ, PET, PRCP, SWE, TAIR, TMIN, TMAX, VP
...
# read in site information file
site_info <- read_csv("F:/ESA/site_info.csv")

# get character vector of site numbers needed
gauges <-  site_info$SITENO

# find files according to filename and watershed id number
...
# select columns for ESA calculation, include SITENO column 
combined_data <-  combined_data %>% dplyr::select(SITENO, DATE, PET, PRCP, AET)

# folder to save updated csv files
dest_dir <- "F:/ESA/ESA_2026"

# create updated csv files for selected watersheds and columns 
...
### ESA STATISTIC ###
#############################################
# input csv files with SITENO, DATE, PRCP, AET, PET columns 
# Matlab code = rdMACH.m

# file path for daily data files 
...
#########################################
# function to calculate ESA statistic 
# Matlab code = QEP.m
...
# daily data can be unaggregated, summed over water year or lowpass filtered and decimated
# Matlab code = QEPanal.m 
...
### CALCULATE ESA ###
##########################################
# function to calculate ESA, combining previous established functions
# runs QEPanal function on a directory
# Matlab code = runQEPanal.m
...
######################################################################
# run all code for directory of files with established fc value (0.5)
# will save "qep_output.csv" file in directory path listed above 
    
runQEPanal(fc = 0.5, dir_path)

#####################################################################
# format output file, results from runQEPanal
# SITENO (basin_00000000_MACH), ESA, P, Ep, E, EP, EpP 
# 747 watersheds, 88 observations per watershed  

# read in csv file
esa_data <- read_csv("F:/ESA_R/qep_output.csv")    

# modify SITENO column values, create DATE, YR and period (Time_Index) columns  
esa_data <- esa_data %>%
  mutate(SITENO = str_extract(SITENO, "\\d{8}")) %>%    
  mutate(across(c(ESA, P, Ep, E, EP, EpP), as.numeric)) %>%       
  group_by(SITENO) %>%
  mutate(DATE = as.Date("1980-01-01") + (row_number() - 1) * 183,
         YR = as.integer(year(DATE)),
         Time_Index = as.integer(row_number())) %>%
  ungroup() %>%
...
### DERIVATIVE ANALYSIS ###
######################################################
# three-point first derivative function, use forward/backward differences on ends 
# Abramowitz, M. and I. A. Stegun, (1972), Handbook of Mathematical Functions, abridged 9th printing, Dover, New York.
# Matlab code = deriv13.m  
...
############################################  
# compute derivatives, save to esa_data file
esa_data <- esa_data %>%
  group_by(SITENO) %>%
  arrange(Time_Index) %>%
  mutate(
    dESA = deriv13(ESA, dx = 0.5)) # dx = 1 for unit spacing (1 to 88), 0.5 for half year  
  
# single file with SITENO, ESA, dESA, P, Ep, E, EP, EpP, DATE, YR, Time_Index
# 88 observations for each site (747 watersheds)
write.csv(esa_data, "F:/ESA_R/esa_data.csv")   
